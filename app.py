...
# 🔴 FORCE CONTEXT (THIS IS THE KEY FIX)
session.sql("USE DATABASE SALES_DB").collect()
...
if st.session_state.section == "Dashboard":
    ...
    FROM SALES_DB.ANALYTICS.SALES_FACT
    WHERE 1=1
    """
    q = apply_filters(q)
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY month ORDER BY month"
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY customer_type"
    df = session.sql(q).to_pandas()
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY product_id ORDER BY revenue DESC LIMIT 5"
    df = session.sql(q).to_pandas()
...
# ==================================================
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY year ORDER BY year"
    df = session.sql(q).to_pandas()
...
# ==================================================
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY order_date ORDER BY order_date"
    df = session.sql(q).to_pandas()
...
# ==================================================
    ...
    WHERE 1=1
    """
    q = apply_filters(q) + " GROUP BY product_id ORDER BY revenue DESC"
    df = session.sql(q).to_pandas()
